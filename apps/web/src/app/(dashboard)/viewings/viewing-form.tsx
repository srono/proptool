...
function parseInitialDateTime(scheduledAt: string) {
...
export function ViewingForm({ leads, listings, preselectedLeadId, googleCalendarConnected, initialData }: Props) {
  const router = useRouter();
  const isEdit = !!initialData?.id;

  const initialDateTime = initialData?.scheduled_at
    ? parseInitialDateTime(initialData.scheduled_at)
    : { date: '', time: '' };

  const [selectedLeadId, setSelectedLeadId] = useState(initialData?.lead_id ?? preselectedLeadId ?? '');
  const [selectedListingId, setSelectedListingId] = useState(initialData?.listing_id ?? '');
  const [date, setDate] = useState(initialDateTime.date);
  const [time, setTime] = useState(initialDateTime.time);
  const [duration, setDuration] = useState(initialData?.duration_mins ?? 60);
  const [syncToCalendar, setSyncToCalendar] = useState(!!googleCalendarConnected);
  const [checklist, setChecklist] = useState<PreViewingChecklist>(() => {
    const lead = leads.find((l) => l.id === (initialData?.lead_id ?? preselectedLeadId));
    return lead?.pre_viewing_checklist ?? DEFAULT_CHECKLIST;
  });
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (error) {
          ...
          alert('Failed to update viewing. Please try again.');
          return;
        }
      } else {
        // Create new viewing
  ...
  return (
    <div className="space-y-6">
          ...
          {showChecklistWarning && (
            <div className="rounded-xl bg-status-amber/10 border border-status-amber/40 p-3">
              ...
              <div className="flex gap-2 mt-3">
                ...
                >
                  {isEdit ? 'Skip and Update Anyway' : 'Skip and Book Anyway'}
                </button>
                <button
                  type="button"
      ...
      {/* Google Calendar sync — only for new viewings */}
      {!isEdit && (
